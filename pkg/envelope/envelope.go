// Package envelope defines the universal data contract between pipes.
//
// This package provides the public API for the envelope types that both
// core and cloud code import.
package envelope
...
// Envelope is the universal data contract passed between pipes.
// Every pipe receives an Envelope and returns an Envelope.
...
// Error carries structured error information within an envelope.
type Error struct {
...
// MemoryEntry is a memory context entry injected into an envelope.
...
// New creates a new Envelope with the given pipe name and action.
...
// FatalError returns a non-retryable fatal Error.
...
// WarnError returns a warning-level Error.
func WarnError(message string) *Error {
	return &Error{
		Message:  message,
...
// ClassifyError wraps err into an Error, marking timeouts as retryable.
func ClassifyError(prefix string, err error) *Error {
	if isTimeout(err) {
		return &Error{
...
// ContentToText renders envelope content as a plain text string based on type.
