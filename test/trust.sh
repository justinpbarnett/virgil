...
if [ "${VIRGIL_TEST_EXTERNAL:-0}" != "1" ]; then
  ...
  if [ -z "${ANTHROPIC_API_KEY:-}" ]; then
    ...
    # Test 10: error recovery rolls back trust (via explicit CLI rollback as proxy)
    # Approve calendar_create to threshold
    for i in $(seq 1 5); do
      $VIRGIL trust approve --action calendar_create --config "$CONFIG" > /dev/null 2>&1
    done
    # Score is now 5 (at threshold); rollback decrements to 4
    # Simulate rollback by running trust reject (which represents what happens internally on error)
    $VIRGIL trust reject --action calendar_create --config "$CONFIG" > /dev/null 2>&1
    if $VIRGIL trust check --action calendar_create --config "$CONFIG" 2>/dev/null; then
      fail "trust check should be blocked after rollback"
    fi
    pass "trust rollback drops score below threshold"
  fi
fi
